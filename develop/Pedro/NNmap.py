...
number_of_inputs = 4
number_of_neurons_hidden_layer = 5
number_of_outputs = 1
...
for pos in range(1,num_of_layers):
    list_of_pos_x.append(list_of_pos_x[pos-1] + delta_x)

#------------------------------------------------------------
# draw circles
for i, y1 in enumerate(np.linspace(max_in_y, min_in_y, number_of_inputs)):
    draw_circle(ax, (x1, y1), radius)
    ax.text(x1 - 0.9, y1, 'Input #%i' % (i + 1),
            ha='right', va='center', fontsize=16)
    draw_connecting_arrow(ax, (x1 - 0.9, y1), 0.1, (x1, y1), radius)

for y2 in np.linspace(max_hidden_y, min_hidden_y, number_of_neurons_hidden_layer):
    draw_circle(ax, (x2, y2), radius)

if number_of_outputs > 1:
    for y3 in np.linspace(max_out_y, min_out_y, number_of_outputs):
        draw_circle(ax, (x3, y3), radius)

else:
    draw_circle(ax, (x3, 0), radius)
